...
const adminLogin = asyncHandler(async (req, res, _next) => {
    const { email, password } = req.body
    if (!email) {
        throw appError.create('Neither Email nor password can be empty.', 400, ERROR)
    }
...
const handleRefreshToken = asyncHandler(async (req, res, _next) => {
        const { _id } = req.user
        validateMongoID(_id)
        const user = await User.findById(_id)
        //console.log(user)
...
const callbackRedirect = asyncHandler(async (req, res, _next) => {
