...
import { ERROR, SUCCESS } from "../utils/errorText.js";
import asyncHandler from "express-async-handler";
import { validateMongoID } from "../utils/validateMongoID.js";

const getAllAds = asyncHandler(async (req, res, _next) => {
...
const getSingleAd = asyncHandler(async (req, res, _next) => {
  const { id } = req.params;
  validateMongoID(id);
...
const createAd = asyncHandler(async (req, res, _next) => {
  const { _id } = req.user;
  validateMongoID(_id);
  ...
  const { name, gov, region, lang, lat } = req.body;
  ...
  if (!enteredRegion) {
    ...
    enteredRegion = await Region.create({
      cityNameAr: region,
...
const editAd = asyncHandler(async (req, res, _next) => {
  const { _id } = req.user;
  const { id } = req.params;
  validateMongoID(id);
  validateMongoID(_id);
  ...
  if (isTheOwner) {
    ...
    if (pics.length) {
      ...
      ad = await Ad.findByIdAndUpdate(
        id,
        { ...req.body, pictures: [...ad.pictures, ...urls] },
        { new: true }
      );
    } else {
...
const deleteAd = asyncHandler(async (_req, _res, _next) => {});
